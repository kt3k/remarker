...
describe('remarker', () => {
  it('creates index.html from slides.md', () => {
    ...
    expect(readFileSync(join(examples.simple, 'build', 'index.html')).toString()).to.include('My Awesome Presentation')
  })

  it('replaces stylesheets by remarker.yml\'s css property', () => {
    execSync('node ../../index.js build', { cwd: examples.remark })

    expect(readFileSync(join(examples.remark, 'build', 'index.html')).toString()).to.include('@import url(https://fonts.googleapis.com/css?family=Droid+Serif);')
  })

  it('replaces remark.js by remarker.yml\'s remarkPath', () => {
    execSync('node ../../index.js build', { cwd: examples.replaceRemark })

    expect(readFileSync(join(examples.replaceRemark, 'build', 'remark.js')).toString()).to.include('console.log("remark.js")')
  })
  ...
  it('injects scripts by remarker.yml\'s script', () => {
    ...
    expect(readFileSync(join(examples.replaceRemark, 'build', 'index.html')).toString()).to.include('console.log("injected script")')
  })

  describe('-s, --source option', () => {
    it('specifies the slide\'s markdown path', () => {
      execSync('node ../../index.js build --source my-slides.md', { cwd: examples.simple })

      expect(readFileSync(join(examples.simple, 'build', 'index.html')).toString()).to.include('This is my-slides.md')

      execSync('node ../../index.js build -s my-slides-2.md', { cwd: examples.simple })

      expect(readFileSync(join(examples.simple, 'build', 'index.html')).toString()).to.include('This is my-slides-2.md')
    })
  })
